...
| `strict-syntax-stops-execution` | syntax | covered baseline | strict POSIX parse diagnostic, status 2, and no non-interactive execution before or after the malformed command |
...
| `builtin-dot-{file-not-found,path-no-cwd-fallback,non-readable}` | special builtin | covered baseline | dot file lookup and permission diagnostics, status 1, non-interactive execution stops |
