...
| `errors-expansion` | supported | high | nounset, `${parameter:?word}`, malformed braced parameter substitutions, arithmetic, redirection-target, assignment-word, for-list, case subject/pattern, command-substitution, and interactive unit coverage | expansion failures exit non-interactive execution in current-shell contexts; failures inside command-substitution subshells exit only that subshell and propagate diagnostics/status; interactive execution aborts the current command without setting shell exit |
