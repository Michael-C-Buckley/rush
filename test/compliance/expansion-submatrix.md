...
- Extension-mode support should eventually cover string-oriented substring `${parameter:offset[:length]}`, replacement `${parameter/pattern/repl}`, case modification `${parameter^}`/`${parameter,}`, indirect expansion `${!name}`, and name-prefix enumeration `${!prefix*}` forms. Array-style parameter operations such as `${name[index]}` belong to a later Bash-mode array syntax slice after array assignment/indexing semantics are designed. Rush currently rejects representative cases with `parameter: bad substitution` negative-corpus coverage.
