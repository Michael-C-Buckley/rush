...
- Extension-mode support should eventually cover string-oriented substring `${parameter:offset[:length]}`, replacement `${parameter/pattern/repl}`, case modification `${parameter^}`/`${parameter,}`, indirect expansion `${!name}`, and name-prefix enumeration `${!prefix*}` forms. Bash mode now has an indexed-array slice for arithmetic `name[index]=word` assignment subscripts, including unquoted whitespace inside assignment subscripts, compound assignment, `${name[index]}` expansion subscripts, whole-array `${name[@]}`/`${name[*]}` expansion, array length/key operations, individual element unset, and negative relative subscripts against the array runtime model. POSIX/default mode still rejects representative Bash array parameter forms with `parameter: bad substitution` coverage.
