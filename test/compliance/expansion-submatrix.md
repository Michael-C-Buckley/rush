...
| Arithmetic expansion | `expansion-arithmetic` | baseline | POSIX diagnostic behavior for invalid/nonnumeric expressions and exact nested legacy-backquote diagnostics |
...
- Bash-mode support covers scalar string-oriented substring `${parameter:offset[:length]}`, replacement `${parameter/pattern/repl}` plus supported global/anchored variants, ASCII case modification `${parameter^}`/`${parameter^^}`/`${parameter,}`/`${parameter,,}`, indirect expansion `${!name}`, and name-prefix enumeration `${!prefix*}` forms. Bash mode also has an indexed-array slice for arithmetic `name[index]=word` assignment subscripts, including unquoted whitespace inside assignment subscripts, compound assignment, `${name[index]}` expansion subscripts, whole-array `${name[@]}`/`${name[*]}` expansion, array length/key operations, individual element unset, and negative relative subscripts against the array runtime model. POSIX/default mode still rejects representative Bash-only parameter forms with `parameter: bad substitution` coverage.
...
- `expansion-arithmetic-short-circuit`
- `expansion-arithmetic-zero-divisor`

Current coverage includes precedence, variable lookup, assignment side effects, compound assignment, comparisons, logical, bitwise, shifts, ternary, comma operator support, octal/hex constants, signed 64-bit boundary constants, logical/conditional short-circuit behavior, and POSIX arithmetic-expression preprocessing of nested parameter expansions and command substitutions before evaluation. Rush's POSIX-first arithmetic values are signed 64-bit integers: decimal/octal/hex constants and variable values must fit `[-9223372036854775808, 9223372036854775807]`, `+`, `-`, `*`, unary `-`, compound assignment, and the `INT64_MIN / -1` or `INT64_MIN % -1` cases report `arithmetic overflow`, and bitwise/shift operators operate on the signed two's-complement representation with masked shift counts. Logical `&&`, logical `||`, and conditional `?:` operators parse but do not evaluate unused operands, so skipped division by zero, signed overflow, and assignment side effects are not observed. The recursive preprocessing behavior was checked against dash, bash `--posix`, and yash for braced defaults, unbraced `$name`, command substitution output, legacy backquote output, backslash-newline continuation, and expression-valued parameter text produced by explicit parameter expansion. Variable values referenced by arithmetic identifiers are intentionally narrower: POSIX integer constants are accepted after trimming leading and trailing shell blank characters, unset/null/blank-only variables evaluate as zero, and nonnumeric values, expression-valued strings, or literal nested substitution text in variable values fail instead of being recursively evaluated. Negative coverage includes invalid operators, missing operands, malformed parentheses, malformed conditional/comma forms, invalid variable values, arithmetic overflow, division or remainder by zero, quoted arithmetic tokens, quote bytes that remain in the expression, escaped `$`/`${...`/backquote initiators that must stay literal, unmatched raw legacy-backquote diagnostics after escaped literal backquotes, malformed parameter syntax inside arithmetic, and `${parameter:?word}` failures inside arithmetic. Invalid arithmetic expansion in a current-shell expansion context, including an assignment word, stops non-interactive execution; inside command substitution it exits only the substitution subshell and propagates diagnostics/status. Division and remainder by zero use the same arithmetic expansion error path and report `division by zero`; dash, bash `--posix`, yash, ksh, and zsh-compatible sh modes agree on the diagnostic class and non-interactive stop behavior, but differ on exact diagnostic wording and status, so Rush pins its POSIX-negative behavior rather than adding it to the differential corpus.

Remaining gaps:

- exact POSIX diagnostic wording for less-common arithmetic syntax failures.

Portability note: POSIX only requires signed long arithmetic and leaves latitude around overflow. dash, bash `--posix`, ksh, and zsh-compatible sh modes often wrap or saturate representative 64-bit overflow cases, while yash diagnoses several overflows. Rush follows the existing evaluator's diagnostic path and treats signed overflow as an arithmetic expansion error instead of adopting a wrapping extension. dash rejects expression-valued arithmetic variables such as `x='1 + 2'; echo $((x))`, bash `--posix` recursively evaluates them, and yash prints the raw value in the audited mode. Rush follows the POSIX-required integer-constant subset and treats expression-valued variable contents as invalid rather than adopting one divergent extension.
