...
| `set` | `option-set`, `option-set-short-clusters`, `option-set-allexport`, `builtin-set-positionals`, `option-set-usage-errors`, option rows | shell option, allexport, clustered short option, set - option reset, no-operand variable listing, and set -- positional parameter corpus; invalid option diagnostics | many POSIX flags and broader operand forms |
