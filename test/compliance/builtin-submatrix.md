...
| `command` | `builtin-command`, `builtin-command-function-suppression`, `builtin-command-option-terminator`, `builtin-command-usage-errors` | `-p`, `-v`, `-V`, `--`, function lookup suppression, option and lookup operand diagnostics | edge cases around special builtins, utilities, PATH errors |
