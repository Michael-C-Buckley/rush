...
| `trap` | `builtin-trap`, `signal-trap-real` | listing, clear, EXIT, INT signal corpus, invalid signal diagnostics; interactive INT trap dispatch is covered by `signal-interactive`, and process-directed trapped signals wake and redraw the active editor | ignored-signal and inheritance semantics outside the supported interactive slice |
