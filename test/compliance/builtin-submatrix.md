...
| `command` | `builtin-command`, `builtin-command-option-terminator`, `builtin-command-usage-errors` | `-p`, `-v`, `-V`, `--`, lookup suppression, option and lookup operand diagnostics | edge cases around special builtins, functions, utilities, PATH errors |
...
| `getopts` | `builtin-getopts`, `builtin-getopts-explicit-args`, `builtin-getopts-option-terminator`, `builtin-getopts-usage-errors` | clusters, required args, silent missing arg, explicit args, `--` termination, invalid optstring and variable-name diagnostics | remaining OPTIND reset edge cases |
| `printf` | `builtin-printf`, `builtin-printf-usage-errors`, printf subrows | escapes, `%b`/`%c`, format reuse, octal/hex, width/precision, missing format, invalid format/conversion, invalid numeric operand diagnostics | full format grammar |
| `read` | `builtin-read`, read subrows | backslash, `-r`, custom IFS, last-variable remainder assignment, EOF status, unsupported option diagnostic | additional IFS edge cases, prompts if ever added |
| `test`, `[` | `builtin-test`, test subrows | file predicates including symlinks, terminal fds, mode bits, and special file types, string comparisons, integer comparisons, ordering, invalid expression | complete POSIX expression grammar, precedence edge cases |
