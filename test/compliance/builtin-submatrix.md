...
| `trap` | `builtin-trap`, `signal-trap-real` | listing, clear, EXIT, INT signal corpus, invalid signal diagnostics; interactive INT trap dispatch is covered by `signal-interactive` | broader signal timing, ignored-signal, and inheritance semantics outside the supported interactive slice |
