...
//!
//! Vendored from Zig 0.16.0 `lib/compiler/test_runner.zig` with one fix: the
//! upstream copy passes `@errorReturnTrace()` (a `*std.builtin.StackTrace`)
//! to `std.debug.writeStackTrace`, which expects `*const std.debug.StackTrace`
//! and fails to compile in fuzz mode (`zig build fuzz --fuzz`). The patched
//! line calls `std.debug.writeErrorReturnTrace` instead. Drop this file once
//! the bundled test runner compiles under `--fuzz`.
