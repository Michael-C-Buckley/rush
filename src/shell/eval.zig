...
const SourceLowerer = struct {
    ...
    fn statementSource(self: *SourceLowerer, program: ir.Program, statement_index: usize) ![]const u8 {
        ...
        const end = if (statementNeedsEmbeddedHereDocSource(program, statement))
            statementBroadEnd(program, statement_index)
        else
            statementSyntacticEnd(program, statement);
        std.debug.assert(start < end);
        std.debug.assert(end <= program.source.len);
        var source: std.ArrayList(u8) = .empty;
        errdefer source.deinit(self.allocator);
        try source.appendSlice(self.allocator, program.source[start..end]);
        try self.appendStatementHereDocRanges(&source, program, statement, end);
        return source.toOwnedSlice(self.allocator);
    }

    fn statementSyntacticEnd(program: ir.Program, statement: ir.Statement) usize {
        return switch (statement.kind) {
    ...
    fn statementBroadEnd(program: ir.Program, statement_index: usize) usize {
        std.debug.assert(statement_index < program.statements.len);
        if (statement_index + 1 < program.statements.len) return program.statements[statement_index + 1].span.start;
        return program.source.len;
    }

    fn statementNeedsEmbeddedHereDocSource(program: ir.Program, statement: ir.Statement) bool {
        return switch (statement.kind) {
            .if_command => ifCommandBodyMayHaveHereDoc(program.if_commands[statement.index]),
            .loop_command => sourceMayHaveHereDoc(program.loop_commands[statement.index].condition) or
                sourceMayHaveHereDoc(program.loop_commands[statement.index].body),
            .for_command => sourceMayHaveHereDoc(program.for_commands[statement.index].body),
            .case_command => caseCommandBodyMayHaveHereDoc(program.case_commands[statement.index]),
    ...
    fn ifCommandBodyMayHaveHereDoc(command: ir.IfCommand) bool {
        for (command.branches) |branch| {
            if (sourceMayHaveHereDoc(branch.condition) or sourceMayHaveHereDoc(branch.body)) return true;
        }
        if (command.else_body) |body| if (sourceMayHaveHereDoc(body)) return true;
        return false;
    }

    fn caseCommandBodyMayHaveHereDoc(command: ir.CaseCommand) bool {
        for (command.arms) |arm| if (sourceMayHaveHereDoc(arm.body)) return true;
        return false;
    }
    ...
    fn appendStatementHereDocRanges(
        self: *SourceLowerer,
        ...
        switch (statement.kind) {
            .pipeline => {
                ...
                for (pipeline.command_indexes) |command_index| {
                    try self.appendRedirectionHereDocRanges(
                        source,
                        program,
            ...
            .if_command => try self.appendRedirectionHereDocRanges(
                source,
                program,
                program.if_commands[statement.index].redirections,
                base_end,
            ),
            .loop_command => try self.appendRedirectionHereDocRanges(
                source,
                program,
                program.loop_commands[statement.index].redirections,
                base_end,
            ),
            .for_command => try self.appendRedirectionHereDocRanges(
                source,
                program,
                program.for_commands[statement.index].redirections,
                base_end,
            ),
            .case_command => try self.appendRedirectionHereDocRanges(
                source,
                program,
            ...
            .subshell => try self.appendRedirectionHereDocRanges(
                source,
                program,
    ...
    fn appendRedirectionHereDocRanges(
        self: *SourceLowerer,
        source: *std.ArrayList(u8),
        program: ir.Program,
