---
name: releasing-rush
description: Prepares and tags Rush releases using semantic versions, checked-in release notes, and guarded verification. Use when drafting release notes, cutting a release, creating a version tag, or checking release readiness.
---

# Releasing Rush

Prepare portable Rush releases whose source of truth is a Git commit, a signed
semantic-version tag, and checked-in Markdown release notes. Publishing is the
responsibility of the repository's configured CI.
...
5. Find the previous reachable Rush release tag with:
...
Keep them identical. Rush derives development identifiers from Git:
...
zig build compile-check --summary all
zig build lint --summary all
zig build test --summary all
zig build conformance --summary all
zig build install -Doptimize=ReleaseSafe -Dregister-shell=false --summary all
```

Report the exact pass, failure, and skip counts; do not hide known warnings.
Then inspect the binary version:

```sh
./zig-out/bin/rush --version
```
...
The output must be exactly `rush X.Y.Z`. Inspect the tag signature and target
before proposing a push. If verification fails, stop. Do not move or delete the
...
version newer than `X.Y.Z`; otherwise Rush intentionally rejects a development
build whose intended version is not newer than its nearest tag.
