...
- Non-POSIX parameter expansion extension forms are excluded from POSIX scoring and tracked separately; representative unsupported substring, replacement, case modification, indirect expansion, name-prefix, transform-flag, and array-index forms currently diagnose `parameter: bad substitution` in the negative corpus.
...
- Non-POSIX extension forms are intentionally outside the POSIX claim. Rush should eventually support string-oriented substring `${parameter:offset[:length]}`, replacement `${parameter/pattern/repl}`, case modification `${parameter^}`/`${parameter,}`, and indirect/name-prefix operations `${!name}`/`${!prefix*}` in an extension mode. Array-style parameter operations such as `${name[index]}` belong to a later Bash-mode array syntax slice after array assignment/indexing semantics are designed. Transformation flags such as `${parameter@Q}` are not in Rush's planned extension-mode scope for now; keep them as unsupported negative coverage until a concrete compatibility use case justifies design work. Representative extension forms currently reject with bad-substitution diagnostics and are tracked in `extensions-parameter-expansion` instead of counted as POSIX gaps.
