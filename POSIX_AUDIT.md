...
- POSIX builtins now include supported `umask`, `times`, `trap`, and `getopts` plus baseline `command`, `eval`, `exec`, `exit`, `readonly`, `shift`, `wait`, `alias`, `unalias`, `jobs`, `fg`, `bg`, and `kill` coverage.
