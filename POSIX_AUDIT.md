...
- `scripts/check-posix-negative-corpus.sh`: `214` expected-error POSIX cases (`1` Linux-only `/dev/full` case skipped on macOS)
...
- Non-POSIX parameter expansion extension forms are excluded from POSIX scoring and tracked separately; representative unsupported substring, replacement, case modification, indirect expansion, and name-prefix forms currently diagnose `parameter: bad substitution` in the negative corpus.
