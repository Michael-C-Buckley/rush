...
- POSIX builtins now include supported `umask` plus baseline `command`, `eval`, `exec`, `exit`, `readonly`, `shift`, `wait`, `times`, `getopts`, `trap`, `alias`, `unalias`, `jobs`, `fg`, `bg`, and `kill` coverage.
