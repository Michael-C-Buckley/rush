...
- Quote tokenization and quote removal for the POSIX-first surface:
  - single quotes, double quotes, dollar-single-quotes, adjacent quoted/unquoted segments, and empty quotes preserve word unity
  - backslash escapes and backslash-newline continuation
  - dollar-single-quotes process representative POSIX escapes before word expansion while preserving the quoted result from field splitting
...
- Alias expansion is owned by the parser token stream for future input/script slices, covering representative assignment-prefixed, redirection-prefixed, compound-list, command-substitution, alias-produced reserved-word compound-command, eval/dot dynamic-definition, recursive/trailing-blank, and same-read-line definition-timing contexts while preserving here-doc body text.
...
- Quote removal after expansions, including single, double, and dollar-single quotes, escaped spaces and escaped newlines, explicit empty fields, field-splitting suppression, quoted command substitutions including inner quotes and legacy backquotes, nested parameter operator words, recursive function and command-substitution bodies, and case/parameter pattern literalization.
- Here-doc expansion for unquoted delimiters; quoted delimiters suppress expansion.
- Expansion error consequences are covered for current-shell contexts including ordinary words, redirection targets, assignment words, for-loop word lists, case subjects and patterns, nounset, `${parameter:?word}`, malformed or unsupported braced parameter substitutions, invalid `${parameter:=word}` / `${parameter=word}` assignment attempts to positional or special parameters, invalid arithmetic syntax, division or remainder by zero, invalid arithmetic expansion, and invalid arithmetic variable values. Command-substitution expansion failures exit only the substitution subshell while surfacing diagnostics and assignment-only status. Interactive expansion failures abort the current command without exiting the prompt loop.
...
- `printf` supports common conversions/escapes including `%b` precision after escape expansion, signed `%i`, representative signed-integer sign/space flags, integer flag ordering/precedence, octal/hex alternate flags, unsigned/octal/hex width and precision, precision/zero-padding placement including negative sign padding and zero-flag suppression when precision is present, zero-valued integer precision behavior, omitted precision digits after a period, representative floating-point conversions and floating flags under the C locale, POSIX C integer constants, literal percent formatting, and a string width/precision combination slice. POSIX numbered argument conversions remain open because dash and bash `--posix` reject representative numbered formats. Non-portable flag/specifier combinations such as sign flags on `%u` and zero padding on `%s` are excluded from the POSIX evidence; dash/bash compatibility for those combinations is separate extension work.
...
- POSIX option baseline:
  ...
  - POSIX.1-2024 does not standardize shell Emacs command-line editing; it records the historical mode as omitted in the `sh` rationale and leaves non-vi modes as implementation extensions. Rush exposes its default terminal-independent Emacs-style editor through `set -o emacs` / `set +o emacs`: `emacs` is on by default, `set -o emacs` disables `vi`, `set -o vi` disables `emacs`, `set +o emacs` clears the explicit Emacs option state without enabling `vi`, and `set +o vi` restores Rush's default Emacs-style editing option. Representative Emacs-style control/meta bindings are unit-backed.
  - `set -o vi` / `set +o vi` switches the interactive terminal-independent editor between POSIX vi mode and Rush's default Emacs-style mode; vi mode includes insert, command, and replace submodes with beam, block, and underline cursors, plus POSIX history pattern search (`/`, `?`, `n`, `N`), previous-line bigword insertion (`_`), `@` command aliases using enabled `_letter` shell aliases as editing input, `v` external-editor handoff for the current line or a numbered history entry, and a behavior-backed pathname editing slice for `=`, `\`, and `*` on the current shell bigword with implicit `*`, sorted pathname matches, `/` directory marks, quoted/backslash-escaped glob metacharacters treated literally, tilde/parameter/command/arithmetic expansion with unquoted generated field splitting before matching, and replacement quoting that preserves single-quote, double-quote, or backslash-escaped input style when the edited word used quoting.
