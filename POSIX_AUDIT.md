...
- POSIX option baseline:
  ...
  - `set -o vi` / `set +o vi` switches the interactive terminal-independent editor between POSIX vi mode and Rush's default Emacs-style mode; vi mode includes insert, command, and replace submodes with beam, block, and underline cursors, plus POSIX history pattern search (`/`, `?`, `n`, `N`), previous-line bigword insertion (`_`), `@` command aliases using enabled `_letter` shell aliases as editing input, and `v` external-editor handoff for the current line or a numbered history entry.
...
- POSIX vi command-line editing still has edge commands tracked separately, including pathname expansion commands and exact repeat/operator-motion edge behavior beyond the covered behavior-backed slices.
