...
- `printf` supports common conversions/escapes including signed `%i`, representative integer sign/space/alternate flags, unsigned/octal/hex width and precision, precision/zero-padding placement including negative sign padding and zero-flag suppression when precision is present, zero-valued integer precision behavior, omitted precision digits after a period, representative floating-point conversions and floating flags under the C locale, POSIX C integer constants, literal percent formatting, and a string width/precision combination slice, but not other long-tail integer/string format grammar combinations or non-C locale details.
